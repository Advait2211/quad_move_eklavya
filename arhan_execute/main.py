...
class CustomAntEnv(gym.Env):
    def __init__(self, xml_file, max_steps=1000, **kwargs):
        super().__init__()
        # load MuJoCo
...
def run_ppo_training():
    ...
    vis_env = make_env(0)()  # create a single env for rendering
    obs, _ = vis_env.reset()
    done = False
    ...
    plt.title('Evaluation Episode Returns')
    plt.xlabel('Episode')
    plt.ylabel('Return')
    plt.grid(True)
    plt.show()
