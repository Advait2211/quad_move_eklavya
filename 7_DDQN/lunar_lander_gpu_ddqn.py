...
# --- Initialize ---
policy_net = DQN(state_size, action_size).to(device) #A
target_net = DQN(state_size, action_size).to(device) #B
...
for step in tqdm(range(TOTAL_STEPS)):
    ...
    if len(replay_buffer) >= WARMUP_STEPS:
        ...
        with torch.no_grad():
            # 🔹 DDQN CHANGE 1 — Action selection from policy_net (A)
            next_actions = policy_net(next_states).argmax(dim=1, keepdim=True)

            # 🔹 DDQN CHANGE 2 — Action evaluation from target_net (B)
            next_q_values = target_net(next_states).gather(1, next_actions)

            # 🔹 DDQN CHANGE 3 — Target uses action from A, value from B
            y = rewards_b + GAMMA * next_q_values * (1 - dones_b)
...
plt.title(f"DDQN Training on LunarLander-v3 ({NUM_ENVS} envs)")
plt.grid()
plt.show()

torch.save(policy_net.state_dict(), "dqn_lunarlander_parallel.pth")
print("Model saved as ddqn_lunarlander_parallel.pth")