...
for update in tqdm(range(START_UPDATE, MAX_UPDATES), desc="PPO Updates"):
    ...
    completed_returns = np.zeros(NUM_ENVS, dtype=np.float32)
    episode_returns = []
    ...
    for step in range(STEPS_PER_ENV):
        ...
        completed_returns += reward
        for i, d in enumerate(done):
            if d:
                episode_returns.append(completed_returns[i])
                completed_returns[i] = 0.0
